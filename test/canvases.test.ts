...
describe('a canvas is a name and an arrangement', () => {
  test('what is written comes back', () => {
    ...
    editCanvas(db, made.id, {
      ...
      placements: [
        { i: 'roadmap.mapmaker', x: 0, y: 0, w: 5, h: 12, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} },
        { i: 'roadmap.references', x: 5, y: 0, w: 7, h: 20, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} },
      ],
    })
...
describe('what arrives over HTTP is not what is written', () => {
  ...
  test('a rectangle no host could lay out is brought back into range', () => {
    ...
    expect(listCanvases(db)[0]?.placements[0]).toEqual({ i: 'a.one', x: 0, y: 0, w: 1, h: 400, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} })
  })
  ...
  test('a number that is not one does not become NaN in the database', () => {
    ...
    expect(listCanvases(db)[0]?.placements[0]).toEqual({ i: 'a.one', x: 0, y: 0, w: 1, h: 5, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} })
  })
})
...
describe('following the module’s height is a property of one container', () => {
  ...
  test('it is written and read back', () => {
    ...
    editCanvas(db, made.id, { placements: [{ i: 'a.one', x: 0, y: 0, w: 6, h: 10, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} }] })
    expect(listCanvases(db)[0]?.placements[0]?.grow).toBe(false)
  })
  ...
  test('the same module can follow on one canvas and hold its size on another', () => {
    ...
    editCanvas(db, two.id, { placements: [{ i: 'a.shared', x: 0, y: 0, w: 6, h: 10, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} }] })
...
describe('a database written before a column existed still opens', () => {
  test('the column is added and what was already stored keeps its meaning', () => {
    ...
    expect(canvas?.placements[0]).toEqual({ i: 'a.old', x: 0, y: 0, w: 6, h: 10, grow: false, pinned: false, prompt: '', promptFor: null, collapsed: false, openH: null, selected: false, filters: {} })
...
describe('a container folded down to its header', () => {
  ...
  test('anything but a literal true leaves the container open', () => {
    ...
    editCanvas(db, made.id, {
      placements: [
        { i: 'a.one', x: 0, y: 0, w: 6, h: 10, collapsed: 'yes' as unknown as boolean },
      ],
    })
